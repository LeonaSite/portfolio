...
const DepthCarousel = ({
  ...
  const tweenTo = useCallback(
    (target, animate) => {
      ...
      const durationValue =
        animate && !reducedRef.current ? cfg.duration / 1000 : 0;
    ...
    >
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className="depth-carousel-arrow depth-carousel-arrow--prev"
      ...
      {showIndicators && count > 1 && (
        <div className="depth-carousel-dots" role="tablist" aria-label="Slides">
          {data.map((_, index) => (
            <button
              key={index}
              type="button"
